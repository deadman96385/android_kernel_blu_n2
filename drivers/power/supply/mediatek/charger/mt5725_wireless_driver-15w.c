...
static struct class * entx_class;
...
{
	if (mte->primary_charger) {
		if (poll_en) {
			usbotg_alarm_start_timer(mte->primary_charger);
			mte->primary_charger->polling_enabled = true;
		} else {
			mte->primary_charger->polling_enabled = false;
			alarm_try_to_cancel(&mte->primary_charger->otg_timer);
		}
	}
}
*/
int usb_otg_set_vbus(int is_on)
{
	...
	if(mte->primary_charger == NULL){
		pr_err("primary_charger get failed %s----\n",__func__);
		return -1;
	}

#if CONFIG_MTK_GAUGE_VERSION == 30
	if (is_on) {
		charger_dev_enable_otg(mte->primary_charger, true);
		charger_dev_set_boost_current_limit(mte->primary_charger,1500000);
		charger_dev_kick_wdt(mte->primary_charger);
		//enable_boost_polling(true);
	} else {
		charger_dev_enable_otg(mte->primary_charger, false);
...
int set_trxset_gpio(int en){
	...
	if (mte->one_pin_ctl){
		if (gpio_is_valid(mte->trxset_gpio)){
			if (en){
				gpio_direction_output(mte->trxset_gpio,1);
			}else{
...
int set_charge_switch_gpio(int en){

	int ret =0;
	
	if (mte->one_pin_ctl){
		if (gpio_is_valid(mte->charge_switch_gpio)){
			if (en){
				gpio_direction_output(mte->charge_switch_gpio,1);
			}else{
				gpio_direction_output(mte->charge_switch_gpio,0);
			}
		}
		return ret;
	}
...
static ssize_t enable_tx_store(struct class *class, struct class_attribute *attr,	const char *buf, size_t count)
{
    int error;
    unsigned int temp;
...
//static DEVICE_ATTR(enabletx, 0664, NULL, enable_tx_store);

static struct class_attribute entx_class_attrs[] = {
	__ATTR(entx, S_IRUGO | S_IWUSR, NULL, enable_tx_store),
	__ATTR_NULL,
};

static int mt5725_entx_sysfs_create(void)
{
	int i = 0,ret = 0;
	entx_class = class_create(THIS_MODULE, "entx");
	if (IS_ERR(entx_class))
		return PTR_ERR(entx_class);
	for (i = 0; entx_class_attrs[i].attr.name; i++) {
		ret = class_create_file(entx_class,&entx_class_attrs[i]);
		if (ret < 0)
		{
			pr_err("mt5725_entx_sysfs_create error !!\n");
			return ret;
		}
	}
	return 0;
...
void MT5725_irq_handle(void) {
    ...
    /* Prize HanJiuping removed 20210616 end */
    /* Skip re-checking missing interrupt since firmware had fixed the bug */
...
static int MT5725_parse_dt(struct i2c_client *client, struct MT5725_dev *mt5725)
{
	...
	mt5725->chipen_gpio = of_get_named_gpio(client->dev.of_node, "chipen_gpio", 0);
	if (gpio_is_valid(mt5725->chipen_gpio)) {
		dev_info(&client->dev,"%s: chipen_gpio:%d\n", __func__, mt5725->chipen_gpio);
		ret = devm_gpio_request(&client->dev, mt5725->chipen_gpio, "chipen_gpio");
		if (ret < 0) {
			dev_err(&client->dev, "%s: chipen-gpio request fail(%d)\n",__func__, ret);
			//return ret;
		}
	} else {
		dev_err(&client->dev,"get chipen-gpio fail %d\n", mt5725->chipen_gpio);
		//return -EINVAL;
	}
#if defined (CONFIG_PRIZE_REVERE_CHARGING_MODE)		
	mt5725->trxset_gpio = of_get_named_gpio(client->dev.of_node, "trxset_gpio", 0);
	if (gpio_is_valid(mt5725->trxset_gpio)) {
		dev_info(&client->dev,"%s: trxset_gpio:%d\n", __func__, mt5725->trxset_gpio);
		ret = devm_gpio_request(&client->dev, mt5725->trxset_gpio, "trxset_gpio");
		if (ret < 0) {
			dev_err(&client->dev, "%s: trxset_gpio request fail(%d)\n",__func__, ret);
			//return ret;
		}
	} else {
		dev_err(&client->dev,"get trxset_gpio fail %d\n", mt5725->trxset_gpio);
		//return -EINVAL;
	}
	
	mt5725->charge_switch_gpio = of_get_named_gpio(client->dev.of_node, "charge_switch_gpio", 0);
	if (gpio_is_valid(mt5725->charge_switch_gpio)) {
		dev_info(&client->dev,"%s: charge_switch_gpio:%d\n", __func__, mt5725->charge_switch_gpio);
		ret = devm_gpio_request(&client->dev, mt5725->charge_switch_gpio, "charge_switch_gpio");
		if (ret < 0) {
			dev_err(&client->dev, "%s: charge_switch_gpio request fail(%d)\n",__func__, ret);
			//return ret;
		}
	} else {
		dev_err(&client->dev,"get charge_switch_gpio fail %d\n", mt5725->charge_switch_gpio);
		//return -EINVAL;
	}
...
/* Prize HanJiuping added 20210706 for GIGASET customized charge restriction feature end */
	...
	mte->primary_charger = get_charger_by_name("primary_chg");
	if (!mte->primary_charger) {
		pr_err("%s: get primary charger device failed\n", __func__);
		//return -ENODEV;
	}
	mt5725_entx_sysfs_create();
#endif
