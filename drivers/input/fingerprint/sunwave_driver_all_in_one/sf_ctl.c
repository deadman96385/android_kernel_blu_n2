...
/* prize added by mahuiyin, goodix-gf5118m fingerprint info, 20190408-start */
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
        sprintf(current_fingerprint_info.chip,"SW330");
...
static int __init sf_ctl_driver_init(void)
{
    ...
    //spi_register_board_info(spi_board_devs, ARRAY_SIZE(spi_board_devs));	//prize use dts add device
...
device_initcall_sync(sf_ctl_driver_init);
