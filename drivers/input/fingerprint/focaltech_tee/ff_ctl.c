...
//beanpod set uuid
static struct TEEC_UUID vendor_uuid = {0x04190000, 0x0000, 0x0000, {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}};
/*
...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    if (g_log_level <= FF_LOG_LEVEL_DBG) {
        ...
        FF_LOGI("%s(.., %s, ..) invoke.", __func__, cmd_names[_cmd]);
    }
#endif
...
static int gf_get_chipid()
{
    ...
    regulator_set_voltage(vdd_reg, 2800000, 2800000);
    ...
    regulator_enable(vdd_reg);
        ...
        if (fpid < 0) {
            pr_err("%s fpid-gpios not provide\n", __func__);
            goto err2;
        }

        pr_err("%s fpid-gpios %d\n", __func__, fpid);
        devm_gpio_request_one(&pdev->dev, fpid, GPIOF_DIR_IN, "goodix fpid");

        if(gpio_get_value(fpid)){
            pr_err("%s goodix finger module\n", __func__);
            err = 1;
        } else {
            pr_err("%s focaltek finger module\n", __func__);
            err = 0;
        }
    } else {
        pr_err("%s device node mediatek,mt6765-fpc is null\n", __func__);
        goto err1;
    }
...
static int __init ff_ctl_driver_init(void)
{
    ...
    // prize baibo for begin 
    if (gf_get_chipid()) {
        pr_err("%s, ft_driver probe goodix finger module, abort!\n", __func__);
        return err;
    }
    // prize baibo for end 
...
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
	//id=mas_connect();
	//prize by baibo start
	sprintf(current_fingerprint_info.chip,"FT9618");
	//prize by baibo end
