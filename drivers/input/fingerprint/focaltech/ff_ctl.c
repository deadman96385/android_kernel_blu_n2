...
static ff_spidev_info_t ff_spidev_info = {0};
extern short bus_num;
extern unsigned char chip_select;
////////////////////////////////////////////////////////////////////////////////
 ...
 */
ff_log_level_t g_log_level = FF_LOG_LEVEL_ERR;	//__FF_EARLY_LOG_LEVEL;
...
{
	ff_spidev_info.bus = bus_num;
	ff_spidev_info.cs  = chip_select;
...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FF_IOC_INIT_DRIVER: {
        if (g_context->b_driver_inited) {
			...
			FF_LOGE("ff_ctl_free_driver(..) = %d.", err);
        }
        if (!err) {
...
static int __init ff_ctl_driver_init(void)
{
	...
	err = ff_ctl_init_driver();
	if(err != 0) {
		FF_LOGD("focal ff_ctl_init_driver = %d \n", err);
        err = ff_ctl_free_driver();
        g_context->b_driver_inited = false;
    }
	*/
