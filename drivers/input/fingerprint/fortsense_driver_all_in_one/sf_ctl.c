...
//prize add by lipengpeng 20210522 start 
...
static int sf_probe(sf_device_t *dev)
{
    ...
    {
//prize add by lipengpeng 20210802 start 
        /* fortsense define, flow by trustonic */
		struct TEEC_UUID vendor_uuid = {0x07721f0c, 0xd217, 0x4f35, \
            {0x93, 0xb7, 0x57, 0x55, 0xc4, 0x8f, 0x45, 0x04} \
        };
        memcpy(&uuid_fp, &vendor_uuid, sizeof(struct TEEC_UUID));
    }
    xprintk(KERN_ERR, "%s set beanpod isee2.7.0 uuid ok \n", __FUNCTION__);
#endif
//0401c03f c30c 4dd0 00 00 00 00 00 00 01 04
// 0x07721f0c 0xd217 0x4f35 0x93 0xb7 0x57 0x55 0xc4 0x8f 0x45 0x04
//prize add by lipengpeng 20210802 end 
//prize add by lipengpeng  20210522 start 
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
        sprintf(current_fingerprint_info.chip,"SW9722");
        //strcpy(current_fingerprint_info.id, GF_LINUX_VERSION);
        strcpy(current_fingerprint_info.vendor,"huixin");
        strcpy(current_fingerprint_info.more,"fingerprint");
#endif
//prize add by lipengpeng  20210522 end 
