...
static int parse_dt(struct device *dev, struct syna_tcm_board_data *bdata)
{
	...
	const char *name;
	static struct regulator *avdd_reg;


	printk("[wolf] %s\n",__func__);
	// //add by wangfei
	// // avdd_reg = regulator_get(dev, "power"); 
	// avdd_reg = devm_regulator_get(dev, "power");
	// if (IS_ERR_OR_NULL(avdd_reg)){
	// 	r = PTR_ERR(avdd_reg);
	// 	printk("[wolf] Failed to get regulator avdd:%d", r);
	// 	return r;
	// }
	// err = regulator_set_voltage(avdd_reg, 3300000, 3300000);
	// if (err) {
	// 	printk("regulator_set_voltage failed err = %d\n", err);
	// 	return err;
	// }
	// err = regulator_enable(avdd_reg);
	// if (err) {
	// 	printk("Regulator vdd enable failed err = %d\n", err);
	// 	return err;
	// }
	// printk("[wolf] avdd is ok avdd = %d\n",regulator_get_voltage(avdd_reg));
...
static int syna_tcm_spi_probe(struct spi_device *spi)
{
	...
	LOGE(&spi->dev, "syna_tcm_spi_probe enter.\n");
...
{
	printk("syna_tcm_bus_init enter.\n");
