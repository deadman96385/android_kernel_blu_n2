...
/* prize add by xuejian 20211221 for check fod event  */
/* 1: fod event not run syna_tcm_rezero*/
/* 0: run syna_tcm_rezero*/
...
void fts_tp_version_get(char *buf, int buf_size)
{
	...
	LOGI(tcm_hcd->pdev->dev.parent,
			"tp firmware version==0x%x\n",tcm_hcd->app_info.customer_config_id[1]);

	retval = snprintf(buf, PAGE_SIZE,
			"0x%x ", tcm_hcd->app_info.customer_config_id[1]);
	if (retval < 0)
		 goto exit;
exit:
	mutex_unlock(&tcm_hcd->extif_mutex);
...
int tcm_gesture_click_set(u8 state)
{
	int ret = 0;
	...
	if (state) {
		tcm_hcd->wakeup_click_enabled = true;
		tcm_hcd->wakeup_gesture_enabled = true;
		LOGN(tcm_hcd->pdev->dev.parent,"==%d able Gesture click Mode!", tcm_hcd->wakeup_gesture_enabled);
	} else {
		tcm_hcd->wakeup_click_enabled = false;
		if (!tcm_hcd->wakeup_fod_enabled) {
			tcm_hcd->wakeup_gesture_enabled = false;
			 LOGN(tcm_hcd->pdev->dev.parent,"==%d disable Gesture  Mode!", tcm_hcd->wakeup_gesture_enabled);
		}
	}

	LOGN(tcm_hcd->pdev->dev.parent,"===gesture click mode:%d", tcm_hcd->wakeup_click_enabled);
	return ret;
}

int tcm_gesture_fod_set(u8 state)
{
	int retval = 0;
	struct syna_tcm_hcd *tcm_hcd = g_tcm_hcd;
	...
	if (state) {
		tcm_hcd->wakeup_fod_enabled = true;
		tcm_hcd->wakeup_gesture_enabled = true;
		LOGN(tcm_hcd->pdev->dev.parent,"==%d able Gesture fod Mode!", tcm_hcd->wakeup_gesture_enabled);
	} else {
		tcm_hcd->wakeup_fod_enabled = false;
		if (!tcm_hcd->wakeup_click_enabled) {
			tcm_hcd->wakeup_gesture_enabled = false;
			 LOGN(tcm_hcd->pdev->dev.parent,"==%d disable Gesture  Mode!", tcm_hcd->wakeup_gesture_enabled);
		}
	}

	LOGN(tcm_hcd->pdev->dev.parent,"==gesture fod mode:%d", tcm_hcd->wakeup_fod_enabled);

	return retval;
}

int tcm_open_icon_set(u8 state) {
	...
	if (state) {
		tcm_hcd->open_icon = true;
        ret = 1;
		LOGN(tcm_hcd->pdev->dev.parent,"==%d open icon Mode!", tcm_hcd->open_icon);
	} else {
		tcm_hcd->open_icon = false;
		LOGN(tcm_hcd->pdev->dev.parent,"==%d close icon mode", tcm_hcd->open_icon);
	}
	return ret;
}
...
static int syna_tcm_get_regulator(struct syna_tcm_hcd *tcm_hcd, bool get)
{
	...
	if (bdata->pwr_reg_name != NULL && *bdata->pwr_reg_name != 0) {
		...
		{
			printk("[wolf] %s:regulator_set_voltage\n",__func__);
			retval = regulator_set_voltage(tcm_hcd->pwr_reg,3300000,3300000);
			if(retval)
			{
				LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to regulator_set_voltage\n");
			}
			printk("[wolf] avdd is ok avdd = %d\n",regulator_get_voltage(tcm_hcd->pwr_reg));
		}
	}
...
static int syna_tcm_resume(struct device *dev)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to able regulators\n");
	}
	LOGN(tcm_hcd->pdev->dev.parent,
			"===syna_tcm_resume==enable regulators  successed\n");
...
static int syna_tcm_suspend(struct device *dev)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent,"===syna_tcm_suspend===\n");
...
static int syna_tcm_probe(struct platform_device *pdev)
{
	...
	LOGI(&pdev->dev, "TP MODULE ENETER.\n");
	...
	tcm_hcd->wakeup_fod_enabled = true;
	tcm_hcd->open_icon = true;
	tcm_hcd->report_click_show_icon = false;
#ifdef PREDICTIVE_READING
...
/* prize add by kangdong 20210910 start */
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
strcpy(current_tp_info.chip," S3909_FW:0x03");
strcpy(current_tp_info.vendor," Synaptics");
strcpy(current_tp_info.id," 0x00");
strcpy(current_tp_info.more," 1080*2340");
#endif
