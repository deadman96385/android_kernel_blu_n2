...
enum gesture_id {
	...
	GESTURE_DOUBLE_TAP = 0X01,    //double click
	GESTURE_FOD_DOWN = 0X80,
	GESTURE_FOD_UP = 0X81
};
...
static void touch_report(void)
{
	...
	static unsigned int fod_flag = 0;
	...
	LOGI(tcm_hcd->pdev->dev.parent,
		"====touch report in_suspend=%d  wakeup_gesture_enabled=%d  gesture_id=%d report_click_show_icon=%d\n",tcm_hcd->in_suspend,tcm_hcd->wakeup_gesture_enabled,touch_data->gesture_id,tcm_hcd->report_click_show_icon);
	#endif

	 if (tcm_hcd->in_suspend &&(tcm_hcd->wakeup_gesture_enabled && tcm_hcd->wakeup_fod_enabled)&&tcm_hcd->open_icon) {
	 	if (touch_data->gesture_id == GESTURE_FOD_DOWN) {
	 		LOGI(tcm_hcd->pdev->dev.parent,"GESTURE_FOD_DOWN\n");
			fod_flag = 1;
			is_fod = 1;
	 		input_report_key(touch_hcd->input_dev, KEY_GESTURE, 1);//KEY_FORWARD
	 		//input_report_key(touch_hcd->input_dev, BTN_TOUCH, 1);
	 		input_sync(touch_hcd->input_dev);
	 	}

	 	if (touch_data->gesture_id == GESTURE_FOD_UP) {
	 		LOGI(tcm_hcd->pdev->dev.parent,"GESTURE_FOD_UP\n");
	 		input_report_key(touch_hcd->input_dev, KEY_GESTURE, 0);
	 		//input_report_key(touch_hcd->input_dev, BTN_TOUCH, 0);
	 		input_sync(touch_hcd->input_dev);
	 	}
	 }

	if (tcm_hcd->wakeup_gesture_enabled && tcm_hcd->wakeup_click_enabled) {
		if (touch_data->gesture_id == GESTURE_DOUBLE_TAP &&tcm_hcd->in_suspend) {
		LOGI(tcm_hcd->pdev->dev.parent,"GESTURE_SINGTAL_TAP\n");
		is_fod = 0;
	...
	for (idx = 0; idx < touch_hcd->max_objects; idx++) {
		...
		case FINGER:
		case GLOVED_FINGER:
			//prize baibo for filter id4 obj begin
			if (0 == object_data[idx].x_width && 0 == object_data[idx].y_width
				&& 0 == object_data[idx].z)
				break;
			//prize baibo for filter id4 obj end
...
static int touch_set_input_params(void)
{
	...
	input_set_abs_params(touch_hcd->input_dev,
			ABS_MT_TOUCH_MAJOR, 0, 58, 0, 0);
	input_set_abs_params(touch_hcd->input_dev,
			ABS_MT_TOUCH_MINOR, 0, 58, 0, 0);
...
static int touch_set_input_dev(void)
{
	...
	set_bit(KEY_GESTURE, touch_hcd->input_dev->keybit);
	set_bit(KEY_GOTO, touch_hcd->input_dev->keybit);
	input_set_capability(touch_hcd->input_dev, EV_KEY, KEY_WAKEUP);
	input_set_capability(touch_hcd->input_dev, EV_KEY, KEY_GESTURE);
	input_set_capability(touch_hcd->input_dev, EV_KEY, KEY_GOTO);
	//input_set_capability(touch_hcd->input_dev, EV_KEY, BTN_TOUCH);
...
int touch_suspend(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (tcm_hcd->wakeup_gesture_enabled) {
        ...
        if (g_tcm_hcd !=NULL && !g_tcm_hcd->wakeup_fod_enabled) {
                LOGN(tcm_hcd->pdev->dev.parent,
                        "==suspend==DC_ENABLE_TOUCH_AND_HOLD\n");
            retval = tcm_hcd->set_dynamic_config(tcm_hcd,
                DC_ENABLE_TOUCH_AND_HOLD,
                0);
            if (retval < 0) {
                LOGE(tcm_hcd->pdev->dev.parent,
                        "Failed to disable touch and hold\n");
                goto exit;
            }
        }
