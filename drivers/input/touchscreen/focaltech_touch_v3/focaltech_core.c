...
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
    sprintf(current_tp_info.id,"0x%02x%02x",id_h, id_l);
#endif
...
extern int mtkfb_lcm_poweroff_ext(void);
#endif
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    /* TP enter sleep mode */
	//huangjiwu for i2c write incell ft8756 fail --begin
	//if (!ts_data->ic_info.is_incell) {
	//huangjiwu for i2c write incell ft8756 fail --end
