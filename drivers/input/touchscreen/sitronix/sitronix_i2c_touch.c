...
static DEVICE_ATTR(reprobe, 0444, NULL, sitronix_ts_reprobe_store);
...
static void sitronix_io_sync_work_func(struct work_struct *work)
{
	...
	else if(1 == pin_val)
	{
		if(!ctp_sync_io_last_status)
		{
			...
			{
				printk("U\n");
				sitronix_touch_uninit();//此函数会阻塞需要再开一个工作队列?
			}
...
static int sitronix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    /*****************************/
    vtouch = regulator_get(&sitronix_ts_gpts.client->dev, "vtouch");
    if (IS_ERR(vtouch)) {
        ret = PTR_ERR(vtouch);
        vtouch = NULL;
        dev_info(&sitronix_ts_gpts.client->dev,"Regulator get failed vtouch ret=%d\n", ret);
    } else {
        dev_info(&sitronix_ts_gpts.client->dev,"Regulator get vtouch success\n");
    }
    
    ret = regulator_set_voltage(vtouch, 2800000,3300000);
    if (ret) {
        regulator_put(vtouch);
        vtouch = NULL;
        dev_info(&sitronix_ts_gpts.client->dev, "Regulator set 2.8V failed vtouch ret=%d\n", ret);
        return ret;
    } else {
        dev_info(&sitronix_ts_gpts.client->dev, "Regulator set 2.8V success\n");
    }

	ret = regulator_enable(vtouch);
	if (ret) {
		dev_info(&sitronix_ts_gpts.client->dev, "enable vtouch failed\n");
		return ret;
	} else {
        dev_info(&sitronix_ts_gpts.client->dev, "enable vtouch success\n");
    }
...
//#define IRQF_TRIGGER_LOW	0x00000008
		...
		ret = request_irq(sitronix_ts_gpts.client->irq, sitronix_ts_irq_handler, IRQF_TRIGGER_RISING , client->name, &sitronix_ts_gpts);
#else
...
//20160614 for pinctrl
static int sitronix_ts_suspend_rst(struct device *dev)
{
	int ret;
	printk("##%s,line=%d\n",__FUNCTION__,__LINE__);

	// ret = pinctrl_select_state(rst_pin->pinctrl, rst_pin->rst_pin_ts_suspend);
	// if (ret)
		// dev_err(dev, "Can't select rst_pin_ts_suspend state\n");
...
static int sitronix_ts_resume_rst(struct device *dev)
{
	int ret;
	printk("%s,line=%d\n",__FUNCTION__,__LINE__);
	// ret = pinctrl_select_state(rst_pin->pinctrl,rst_pin->rst_pin_ts_active);
	// if (ret)
		// dev_err(dev, "Can't select rst_pin_ts_release state\n");

	printk("%s,line=%d,sitronix_ts_gpts.reset_gpio = %d\n",__FUNCTION__,__LINE__,sitronix_ts_gpts.reset_gpio);
	msleep(50);
	gpio_direction_output(sitronix_ts_gpts.reset_gpio, 0);
	msleep(10);
	gpio_direction_output(sitronix_ts_gpts.reset_gpio, 1);
