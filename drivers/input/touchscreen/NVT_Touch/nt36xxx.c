 ...
 *
 * $Revision: 64124 $
 * $Date: 2020-06-10 18:29:28 +0800 (Wed, 10 Jun 2020) $
 *
...
int32_t nvt_read_pid(void)
{
	...
	NVT_LOG("PID=%04X\n", ts->nvt_pid);
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
    sprintf(current_tp_info.id,"0x%04x",ts->nvt_pid);
#endif
	return ret;
}
...
//prize added by huarui, fts support, 20190111-start
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
    sprintf(current_tp_info.more,"fw_ver=0x%02X",ts->fw_ver);
#endif
//prize added by huarui, fts support, 20190111-end
		NVT_ERR("FW info is ! fw_ver=0x%02X, ~fw_ver=0x%02X\n", buf[1], buf[2]);
...
//prize added by huarui, fts support, 20190111-start
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
	sprintf(current_tp_info.chip,"%02x%02x%02x", buf[6],buf[5],buf[4]);
    strcpy(current_tp_info.vendor,"novatek");
#endif
//prize added by huarui, fts support, 20190111-end
