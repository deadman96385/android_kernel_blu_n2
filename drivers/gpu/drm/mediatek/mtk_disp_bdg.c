...
/***** prize baibo NFC SRCLKENAI0 Interrupt Handler +++ *****/
...
//prize add by wangfei for first suspend 6382 20211011 start
unsigned int first_6382_suspend;
//prize add by wangfei for first suspend 6382 20211011 end
...
void bdg_tx_pull_6382_reset_pin(struct mtk_dsi *dsi)
{
	...
	bdg_reset_gpio = devm_gpiod_get(dev, "bdgreset", GPIOD_OUT_HIGH);//prize modify by wangfei for 6382 reset 20210803
			...
			void *cmdq, struct mtk_dsi *dsi)
{
	...
	bool hstx_cklp_en = true;   //wangfei modify 
...
#else
			BDG_OUTREG32(cmdq, TX_REG[i]->DSI_TX_BUF_CON1,0x0dfd0660);  //prize add by wangfei for 6382 0x0dfd0660 => 0x0dfd0def
#endif
		}
	}
...
int bdg_dsi_dump_reg(enum DISP_BDG_ENUM module)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DDPMSG("INT_ST_MAIN(0x0000d00c): 0x%08x\n", tmp);
		if (tmp != 0) {
			if (tmp & (1 << 0))
				DDPMSG("INT_ST_MAIN(bit0), int_st_phy_fatal\n");
			if (tmp & (1 << 1))
				DDPMSG("INT_ST_MAIN(bit1), int_st_dsi_fatal\n");
			if (tmp & (1 << 2))
				DDPMSG("INT_ST_MAIN(bit2), int_st_ddi_fatal\n");
			if (tmp & (1 << 3))
				DDPMSG("INT_ST_MAIN(bit3), int_st_ipi_fatal\n");
			if (tmp & (1 << 4))
				DDPMSG("INT_ST_MAIN(bit4), int_st_fifo_fatal\n");
			if (tmp & (1 << 16))
				DDPMSG("INT_ST_MAIN(bit16), int_st_phy\n");
			if (tmp & (1 << 17))
				DDPMSG("INT_ST_MAIN(bit17), int_st_dsi\n");
			if (tmp & (1 << 18))
				DDPMSG("INT_ST_MAIN(bit18), int_st_ddi\n");
			if (tmp & (1 << 19))
				DDPMSG("INT_ST_MAIN(bit19), int_st_ipi\n");
			if (tmp & (1 << 21))
				DDPMSG("INT_ST_MAIN(bit21), int_st_err_rpt\n");
			if (tmp & (1 << 22))
				DDPMSG("INT_ST_MAIN(bit22), int_st_rx_triggers\n");
		}

		tmp = mtk_spi_read(0x0000d200);
		if (tmp != 0) {
			DDPMSG("INT_ST_PHY_FATAL(0x0000d200): 0x%08x\n", tmp);
			if (tmp & (1 << 0))
				DDPMSG("Lane 0 Start-of-Transmission Synchronization Error.\n");
			if (tmp & (1 << 1))
				DDPMSG("Lane 0 Escape Entry Error.\n");
			if (tmp & (1 << 2))
				DDPMSG("Lane 0 Control Error.\n");
			if (tmp & (1 << 3))
				DDPMSG("Lane 0 LP Data Transmission Synchronization Error.\n");
			if (tmp & (1 << 4))
				DDPMSG("Lane 0 LP0 Contention Error.\n");
			if (tmp & (1 << 5))
				DDPMSG("Lane 0 LP1 Contention Error.\n");
			if (tmp & (1 << 7)) {
				DDPMSG("Peripheral LPTX timeout defined in DSI ");
				DDPMSG("protocol, avoid to bus contention.\n");
			}
			if (tmp & (1 << 8))
				DDPMSG("Lane 1 Start-of-Transmission Synchronization Error.\n");
			if (tmp & (1 << 9))
				DDPMSG("Lane 1 Escape Entry Error.\n");
			if (tmp & (1 << 10))
				DDPMSG("Lane 1 Control Error.\n");
			if (tmp & (1 << 11)) {
				DDPMSG("Skew between lanes is greater than ");
				DDPMSG("maximum allowed value.\n");
			}
			if (tmp & (1 << 16))
				DDPMSG("Lane 2 Start-of-Transmission Synchronization Error.\n");
			if (tmp & (1 << 17))
				DDPMSG("Lane 2 Escape Entry Error.\n");
			if (tmp & (1 << 18))
				DDPMSG("Lane 2 Control Error.\n");
			if (tmp & (1 << 24))
				DDPMSG("Lane 3 Start-of-Transmission Synchronization Error.\n");
		}

		tmp = mtk_spi_read(0x0000d220);
		if (tmp != 0) {
			DDPMSG("INT_ST_DSI_FATAL(0x0000d220): 0x%08x\n", tmp);
			if (tmp & (1 << 0))
				DDPMSG("Payload Checksum error detected on VC 0.\n");
			if (tmp & (1 << 1))
				DDPMSG("Payload Checksum error detected on VC 1.\n");
			if (tmp & (1 << 2))
				DDPMSG("Payload Checksum error detected on VC 2.\n");
			if (tmp & (1 << 3))
				DDPMSG("Payload Checksum error detected on VC 3.\n");
			if (tmp & (1 << 4)) {
				DDPMSG("LPRX only. Payload size Mismatch error ");
				DDPMSG("detected on VC 0. Causes Payload Checksum error.\n");
			}
			if (tmp & (1 << 5)) {
				DDPMSG("LPRX only. Payload size Mismatch error ");
				DDPMSG("detected on VC 1. Causes Payload Checksum error.\n");
			}
			if (tmp & (1 << 6)) {
				DDPMSG("LPRX only. Payload size Mismatch error ");
				DDPMSG("detected on VC 2. Causes Payload Checksum error.\n");
			}
			if (tmp & (1 << 7)) {
				DDPMSG("LPRX only. Payload size Mismatch error ");
				DDPMSG("detected on VC 3. Causes Payload Checksum error.\n");
			}
			if (tmp & (1 << 16))
				DDPMSG("Header error unrecoverable.\n");
			if (tmp & (1 << 17)) {
				DDPMSG("Read or TE command is followed by another read ");
				DDPMSG("or write command instead of BTA.\n");
			}
			if (tmp & (1 << 18))
				DDPMSG("Invalid data type detected.\n");
		}

		tmp = mtk_spi_read(0x0000d240);
		DDPMSG("INT_ST_DDI_FATAL(0x0000d240): 0x%08x\n", tmp);
		if (tmp & (1 << 0)) {
			DDPMSG("This timeout should rise when the DSI Display panel is not ");
			DDPMSG("outputting response data during programmed value.\n");
			DDPMSG("Data received after this timeout are ignored and ");
			DDPMSG("related logic will be reset.\n");
		}
		if (tmp & (1 << 1))
			DDPMSG("Reports error when panel responds an invalid data type.\n");

		tmp = mtk_spi_read(0x0000d260);
		DDPMSG("INT_ST_IPI_FATAL(0x0000d260): 0x%08x\n", tmp);
		if (tmp & (1 << 0)) {
			DDPMSG("This bit asserts when receiving error IPI timing information, ");
			DDPMSG("e.g. no HSE after HSS in pulse mode.\n");
		}
		if (tmp & (1 << 1)) {
			DDPMSG("This bit asserts when receiving non-integer pixel ");
			DDPMSG("number video packet.\n");
		}
		tmp = mtk_spi_read(0x0000d280);
		if (tmp != 0) {
			DDPMSG("INT_ST_FIFO_FATAL(0x0000d280): 0x%08x\n", tmp);
			if (tmp & (1 << 0))
				DDPMSG("LPRX FIFO overflow, caused by LPRX from PHY.\n");
			if (tmp & (1 << 1))
				DDPMSG("LPTX FIFO overflow, caused by LPTX from DDI.\n");
			if (tmp & (1 << 4))
				DDPMSG("D-PHY HSRX elastbuf overflow, caused by D-PHY HSRX.\n");
			if (tmp & (1 << 8))
				DDPMSG("DDI Header FIFO overflow, caused by HSRX from PPI.\n");
			if (tmp & (1 << 9))
				DDPMSG("DDI Payload FIFO overflow, caused by HSRX from PPI.\n");
			if (tmp & (1 << 16))
				DDPMSG("IPI Control FIFO overflow, caused by HSRX from PPI.\n");
			if (tmp & (1 << 17))
				DDPMSG("IPI Pixel FIFO overflow, caused by HSRX from PPI.\n");
			if (tmp & (1 << 18))
				DDPMSG("IPI Pixel FIFO overflow, caused by read from IPI.\n");
		}

		tmp = mtk_spi_read(0x0000d210);
		if (tmp != 0) {
			DDPMSG("INT_ST_PHY(0x0000d210): 0x%08x\n", tmp);
			if (tmp & (1 << 0))
				DDPMSG("Lane 0 Start-of-Transmission(SoT) Error.\n");
			if (tmp & (1 << 1))
				DDPMSG("Lane 1 Start-of-Transmission(SoT) Error.\n");
			if (tmp & (1 << 2))
				DDPMSG("Lane 2 Start-of-Transmission(SoT) Error.\n");
			if (tmp & (1 << 3))
				DDPMSG("Lane 3 Start-of-Transmission(SoT) Error.\n");
			if (tmp & (1 << 4))
				DDPMSG("HS Invalid Code Word Detection on lane 0.\n");
			if (tmp & (1 << 5))
				DDPMSG("HS Invalid Code Word Detection on lane 1.\n");
			if (tmp & (1 << 6))
				DDPMSG("HS Invalid Code Word Detection on lane 2.\n");
			if (tmp & (1 << 16)) {
				DDPMSG("This timeout is to inform the user when the LPDT ");
				DDPMSG(
					"transmission duration is more than the programmed time.\n");
			}
			if (tmp & (1 << 17)) {
				DDPMSG("This timeout is to inform the user when the high-speed ");
				DDPMSG("reception duration is more than the programmed time.\n");
			}
		}

		tmp = mtk_spi_read(0x0000d230);
		if (tmp != 0) {
			DDPMSG("INT_ST_DSI(0x0000d230): 0x%08x\n", tmp);
			if (tmp & (1 << 0))
				DDPMSG("Header ECC Check error detected on VC 0.\n");
			if (tmp & (1 << 1))
				DDPMSG("Header ECC Check error detected on VC 1.\n");
			if (tmp & (1 << 2))
				DDPMSG("Header ECC Check error detected on VC 2.\n");
			if (tmp & (1 << 3))
				DDPMSG("Header ECC Check error detected on VC 3.\n");
			if (tmp & (1 << 4)) {
				DDPMSG("EoT packet detected error, according the configuration ");
				DDPMSG("EOTP_CFG.\n");
			}
		}
		tmp = mtk_spi_read(0x0000d250);
		if (tmp != 0) {
			DDPMSG("INT_ST_DDI(0x0000d250): 0x%08x\n", tmp);
			if (tmp & (1 << 0)) {
				DDPMSG("This timeout should rise when the DSI Display panel is ");
				DDPMSG(
					"not accepting the incoming data during programmed value.\n");
			}
			if (tmp & (1 << 1)) {
				DDPMSG("This field reports when DSI2 host sends an invalid ");
				DDPMSG("virtual channel for DDI command packets.\n");
			}
			if (tmp & (1 << 4)) {
				DDPMSG("This field reports when received response packet number ");
				DDPMSG("mismatch with the value in WC.\n");
			}
			if (tmp & (1 << 5))
				DDPMSG("This field reports when panel responds an invalid VC.\n");
			if (tmp & (1 << 6)) {
				DDPMSG("This field reports when panel responds data after ");
				DDPMSG("response timeout is fired.\n");
			}
		}

		tmp = mtk_spi_read(0x0000d270);
		DDPMSG("INT_ST_IPI(0x0000d270): 0x%08x\n", tmp);
		if (tmp & (1 << 0)) {
			DDPMSG("This bit asserts when receiving invalid virtual channel packets ");
			DDPMSG("according the configuration IPI_VALID_VC_CFG.\n");
		}
		if (tmp & (1 << 1)) {
			DDPMSG("This bit asserts when frame timing information is corrupted ");
			DDPMSG("because of back pressure.\n");
		}
		tmp = mtk_spi_read(0x0000d2b0);
		if (tmp != 0) {
			DDPMSG("INT_ST_ERR_RPT(0x0000d2b0): 0x%08x\n", tmp);
			if (tmp & (1 << 0))
				DDPMSG("SOT Error.\n");
			if (tmp & (1 << 1))
				DDPMSG("SOT Sync Error.\n");
			if (tmp & (1 << 2))
				DDPMSG("EOT Error.\n");
			if (tmp & (1 << 3))
				DDPMSG("Escape mode Entry Command Error.\n");
			if (tmp & (1 << 4))
				DDPMSG("Low-power Transmit Sync Error.\n");
			if (tmp & (1 << 5))
				DDPMSG("Peripheral Timeout Error.\n");
			if (tmp & (1 << 6))
				DDPMSG("False Control Error.\n");
			if (tmp & (1 << 7))
				DDPMSG("Contention detected.\n");
			if (tmp & (1 << 8))
				DDPMSG("ECC Error, single-bit(detected and corrected).\n");
			if (tmp & (1 << 9))
				DDPMSG("ECC Error, multi-bit(detected and not corrected).\n");
			if (tmp & (1 << 10))
				DDPMSG("Payload Checksum Error(long packet only).\n");
			if (tmp & (1 << 11))
				DDPMSG("DSI Data Type Not Recognized.\n");
			if (tmp & (1 << 12)) {
				DDPMSG("DSI VC ID invalid, both DDI_VALID_VC_CFG & ");
				DDPMSG("IPI_VALID_VC_CFG related error will assert this bit.\n");
			}
			if (tmp & (1 << 13))
				DDPMSG("Invalid Transmission Length.\n");
			if (tmp & (1 << 15))
				DDPMSG("DSI Protocol Violation.\n");
			if (tmp & (1 << 16))
				DDPMSG("High-speed Invalid Code Word Detection on lane.\n");
		}

		tmp = mtk_spi_read(0x0000d2d0);
		if (tmp != 0) {
			DDPMSG("INT_ST_RX_TRIGGERS(0x0000d2d0): 0x%08x\n", tmp);
			if (tmp & (1 << 0))
				DDPMSG("Trigger 0\n");
			if (tmp & (1 << 1))
				DDPMSG("Trigger 1\n");
...
//prize add by wangfei for first suspend 6382 20211011 start
unsigned int get_mt6382_first_suspend_stat(void)
{
	DDPMSG("%s, first_6382_suspend=%d\n", __func__, first_6382_suspend);
	return first_6382_suspend;
}
//prize add by wangfei for first suspend 6382 20211011 end
...
void bdg_first_init(void)
{
	...
	BDG_OUTREGBIT(NULL, struct IRQ_MSK_CLR_SET_REG, SYS_REG->IRQ_MSK_CLR, REG_00, 1);
	BDG_OUTREGBIT(NULL, struct DSI_TX_INTEN_REG,
			TX_REG[0]->DSI_TX_INTEN, BUFFER_UNDERRUN_INT_EN,
			1);
	
	//prize add by wangfei for first suspend 6382 20211011 start
	first_6382_suspend = 1;
	//prize add by wangfei for first suspend 6382 20211011 end
}
...
int bdg_common_deinit(enum DISP_BDG_ENUM module, void *cmdq)
{
	...
	set_LDO_off(cmdq);
	need_6382_init = 1;
	first_6382_suspend = 0; //prize add by wangfei for first suspend 6382 20211011 
...
static int mtk_bdg_eint_check_worker_kthread(void *data)
{
	...
	while (1) {
		...
		if (irq_ctrl3 & BIT(0)) {
			if(mtk_spi_read(0x0000d200) !=0){
				DDPMSG("%s,SOT error\n",__func__);
				bdg_dsi_dump_reg(DISP_BDG_DSI0);
				DDPAEE("disp bdg SOT error\n");
			}
		}
