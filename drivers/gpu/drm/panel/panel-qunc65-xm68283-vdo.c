...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	udelay(10 * 1000);
	gpiod_set_value(ctx->reset_gpio, 1);
	msleep(100);//xm68283 at least 10ms
...
static struct drm_display_mode default_mode = {
	.clock = 99148,//htotal*vtotal*vrefresh/1000   163943   182495
	.hdisplay = HAC,
	.hsync_start = HAC + HFP,
...
static int fb_notifier_callback(struct notifier_block *nb, unsigned long event, void *data){
	...
	if (fb_event && fb_event->data) {
		blank = fb_event->data;
		printk("xm68283 Notifier's event = %ld, blank:%d truly\n", event, *blank);
		switch (*blank) {
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	dsi->mode_flags = MIPI_DSI_MODE_VIDEO | MIPI_DSI_MODE_VIDEO_SYNC_PULSE
			 ...
			 | MIPI_DSI_CLOCK_NON_CONTINUOUS;         //MIPI_DSI_MODE_VIDEO_SYNC_PULSE  MIPI_DSI_MODE_VIDEO_BURST  MIPI_DSI_MODE_LPM
...
MODULE_DESCRIPTION("truly xm68283 VDO LCD Panel Driver");
