...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	LOG_INF("OV8856 get_imgsensor_id in\n");
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				//app_get_front_sensor_name("ov08856_ofilm");
				return ERROR_NONE;
				}
...
UINT32 OV88562lane_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
    LOG_INF("OV8856_MIPI_RAW_SensorInit in\n");
