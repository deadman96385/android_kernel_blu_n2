...
#define PFX "s5kgm1spgms_camera_sensor"
/****************************   Modify end	**************************/
...
#ifdef VENDOR_EDIT
#define DEVICE_VERSION_S5KGM1SPGMS    "s5kgm1spgms"
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[] = {
	...
	{4000, 3000,  0,   0, 4000, 3000, 4000, 3000, 0000, 0000, 4000, 3000, 0, 0, 4000, 3000},	/* custom3 */
};
...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == S5KGM1SPGMS_SENSOR_ID)
				&& (data_base == 0x1640)
				&& (data_length == 1561)) {
...
static kal_uint16 otp_read_module_id(void)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = {(char)(MODULE_ID_OFFSET >> 8), (char)(MODULE_ID_OFFSET & 0xFF)};
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001));
			otp_module_id = otp_read_module_id();
			if(otp_module_id == 0x0)
			{
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				//add by wq 	register_imgsensor_deviceinfo("Cam_b", DEVICE_VERSION_S5KGM1SPGMS, module_id);
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
		LOG_INF("SENSOR_FEATURE_GET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		s5kgm1spgms_get_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32)
					   , feature_data_16);
		break;
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
		 ...
		 *	(*feature_para_len));
		 */
		s5kgm1spgms_set_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32)
					   , feature_data_16);
		break;
#endif
...
UINT32 S5KGM1SPGMS_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
} /* s5kgm1spgms_MIPI_RAW_SensorInit */
