...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 150) { //15fps for PIP
		...
		write_cmos_sensor(0x370b, 0x1b);
		write_cmos_sensor(0x3808, 0x0a);
		write_cmos_sensor(0x3809, 0x20);//2592
		write_cmos_sensor(0x380a, 0x07);
		write_cmos_sensor(0x380b, 0x98);//1944
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				LOG_INF("wangliang: ov5670 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			LOG_INF("wangliang : ov5670 Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
