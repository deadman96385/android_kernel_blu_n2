...
#if 0
static struct agc_params_struct GC8034SUB_AGC_Param[9] = {
	{
		64, /* 1.00x */
		{
			{ 0, 0xfe, {0x00, 0x00} },
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC8034SUB_SENSOR_ID,       /*record sensor id defined in Kd_imgsensor.h*/
...
static void set_dummy(void)
{
	...
	/*hb = imgsensor.dummy_pixel + GC8034SUB_DEFAULT_DUMMY_PIXEL_NUMS;*/
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	for (gain_index = 8; gain_index >= 0; gain_index--)
		if (real_gain >= GC8034SUB_AGC_Param[gain_index].gain_level) {
			write_cmos_sensor(0xb6, gain_index);
			temp_gain = 256 * real_gain / GC8034SUB_AGC_Param[gain_index].gain_level;
			write_cmos_sensor(0xb1, temp_gain >> 8);
			write_cmos_sensor(0xb2, (temp_gain & 0xff));
			for (i = 0; i < AGC_REG_NUM; i++) {
				write_cmos_sensor(GC8034SUB_AGC_Param[gain_index].agc_register[i].addr,
					GC8034SUB_AGC_Param[gain_index].agc_register[i].value[gamma_index]);
			}
			break;
		}
#endif
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				otp_sub_i2c_write_id = imgsensor.i2c_write_id;
				return ERROR_NONE;
			}
...
UINT32 GC8034SUBMIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
