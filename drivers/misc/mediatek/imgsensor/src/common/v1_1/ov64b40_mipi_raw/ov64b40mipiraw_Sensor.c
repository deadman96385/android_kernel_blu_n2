...
#define EEPROM_READ_ID  0xA4 //Prize add by zhuzhengjiang for compatibility of remosaic sensor start
...
static int read_eeprom(int offset, char *data, kal_uint32 size)
{
	...
	for (i = 0; i < size; i++) {
		...
		//printk("get addr:0x%x the value is:0x%x \n",addr,data[i]);
		addr++;
		sum = sum + data[i];
	}
	printk("sum = %d",sum);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			printk("ov64b40 flag=%d \n",flag);
			if(flag != 0) {
			    *sensor_id = 0xFFFFFFFF;
			    printk("it is remosaic sensor,driver is not match module \n");
			}
			/* Prize add by zhuzhengjiang for compatibility of remosaic sensor end */
