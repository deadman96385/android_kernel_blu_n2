...
#define BF2253_SERIAL_ADDR	0x6D
...
//prize modify by yantaotao for camera start
 ...
 extern struct hardware_info current_camera_info[6];
...
int kdCISModulePowerOn(struct i2c_client* client,  bool On)
{
	...
	//gc6133
		if (On){
			...
			gpio_direction_output(dcam_data->dvdd_pin,0);
			mdelay(5);
			gpio_direction_output(dcam_data->pdn_pin,1);
			gpio_direction_output(dcam_data->rst_pin,0);
		}
	}
	else if(client->addr == 0x6D)
	{
	//bf2253
		if (On){
			//set_avdd_regulator(1);
...
static void CameraStreamOn(struct i2c_client *client)
{
	...
	else if(sensor_id == BF2253_SENSOR_ID)
		client->addr = BF2253_SERIAL_ADDR;
	else
		client->addr = 0x3c;

	if(sensor_id != BF2253_SENSOR_ID) {
...
void BF2253_Sensor_Init(struct i2c_client *client)
{
	write_cmos_sensor(client,0xe1, 0x06);
	write_cmos_sensor(client,0xe2, 0x06);
	write_cmos_sensor(client,0xe3, 0x0e);
	write_cmos_sensor(client,0xe4, 0x40);
	write_cmos_sensor(client,0xe5, 0x67);
	write_cmos_sensor(client,0xe6, 0x02);
	write_cmos_sensor(client,0xe8, 0x84);
	write_cmos_sensor(client,0x01, 0x14);
	write_cmos_sensor(client,0x03, 0x98);
	write_cmos_sensor(client,0x27, 0x21);
	write_cmos_sensor(client,0x29, 0x20);
	write_cmos_sensor(client,0x59, 0x10);
	write_cmos_sensor(client,0x5a, 0x10);
	write_cmos_sensor(client,0x5c, 0x11);
	write_cmos_sensor(client,0x5d, 0x73);
	write_cmos_sensor(client,0x6a, 0x2f);//global gain
	write_cmos_sensor(client,0x6b, 0x0e);
	write_cmos_sensor(client,0x6c, 0x7e);//{0x6b,0x6c}int time
	write_cmos_sensor(client,0x6f, 0x10);
	write_cmos_sensor(client,0x70, 0x08);
	write_cmos_sensor(client,0x71, 0x05);
	write_cmos_sensor(client,0x72, 0x10);
	write_cmos_sensor(client,0x73, 0x08);
	write_cmos_sensor(client,0x74, 0x05);
	write_cmos_sensor(client,0x75, 0x06);
	write_cmos_sensor(client,0x76, 0x20);
	write_cmos_sensor(client,0x77, 0x03);
	write_cmos_sensor(client,0x78, 0x0e);
	write_cmos_sensor(client,0x79, 0x08);
...
static UINT32 CameraOpen(struct i2c_client *client)
{
	...
	g_dcam_r_client = kmalloc(sizeof(struct i2c_client), GFP_KERNEL );
	...
	if(!id_status)//prize yanrenjie added gc6153 20190416
	{
		for(i=0;i<3;i++)
		{
			...
			CAMERA_DBG("BF2253 *sensorID=%x func: %s line: %d\n",sensor_id,__func__,__LINE__);
			if(sensor_id != BF2253_SENSOR_ID)
			{
				CAMERA_DBG("BF2253 mipi Read Sensor ID Fail[open] = 0x%x\n", sensor_id);
				//return ERROR_SENSOR_CONNECT_FAIL;
			}else{
				g_dcam_r_client->addr = BF2253_SERIAL_ADDR;
				id_status = 1;
				break;
			}
		}
...
static kal_uint16 Camera_Read_Shutter(struct i2c_client *client)
{
	...
	if(sensor_id == BF2253_SENSOR_ID)
		shutter = shutter * 8;	// prize add by yanrenjie make shutter approximate, bf2253 max shutter is 500
...
static ssize_t show_camera_front_open(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	/* prize hanjiuping 20211210 modified for k6607 sub2cam 8804 CIT fail start
	 * Do real CameraOpen here, since sub2cam may share same power supply with others,
	 * once those camera experience power cycle, then sub2cam would be failed.
	 */
	// if (!dcam_data->is_enabled){
		//kdCISModulePowerOn(client,"camerayuv",true,"kd_camera_hw");
		ret = CameraOpen(client);
...
static int dcam_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id){
	...
	if (!strcmp(client->name,"dcam_r")){
		...
		printk("DCAM dcam_data->avdd_pin(%d)  dcam_data->dvdd_pin(%d) \n",dcam_data->avdd_pin,dcam_data->dvdd_pin);
		gpio_request(dcam_data->pdn_pin,"dcam_f");
		gpio_direction_output(dcam_data->pdn_pin,0);
		gpio_request(dcam_data->rst_pin,"dcam_f");
		gpio_direction_output(dcam_data->rst_pin,0);
		gpio_request(dcam_data->avdd_pin,"dcam_f");
		gpio_direction_output(dcam_data->dvdd_pin,0);
		gpio_request(dcam_data->avdd_pin,"dcam_f");
		gpio_direction_output(dcam_data->dvdd_pin,0);
		ret = of_property_read_u32(node,"sensor_type",&dcam_data->sensor_type);
		...
		//prize modify by yantaotao for camera start
		strcpy(current_camera_info[5].chip,"bf2253Lsub_mipi_raw");
 		sprintf(current_camera_info[5].id,"0x%04x",BF2253_SENSOR_ID);
  	    //strcpy(current_camera_info[5].vendor,"unknow");
		//prize modify by yantaotao for camera end
