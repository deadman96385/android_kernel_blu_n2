...
#define AW36515_DTNAME "mediatek,flashlights_aw36515ir"
#endif
#ifndef AW36515_DTNAME_I2C
#define AW36515_DTNAME_I2C "awinic,aw36515ir"
#endif
#define AW36515_NAME "aw36515ir"
...
static struct aw36515_chip_data *aw36515;
 ...
 *****************************************************************************/
 static const int aw36515_torch_current[AW36515_LEVEL_NUM] = {
	  12,   30,  46, 61, 77, 93, 108,  126,  151,  175,
	 200,  226, 249, 275, 300, 326, 351,0,0,0,
	 0,0,0,0,0,0
};
...
static int aw36515_set_level(int channel, int level)
{
	...
	if (aw36515->pdata->force_level >= 0){
		level = aw36515->pdata->force_level;
	}
...
static int aw36515_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_GET_MAX_TORCH_DUTY:
		...
		fl_arg->arg = AW36515_LEVEL_NUM - 1;
		break;
	case FLASH_IOC_GET_DUTY_CURRENT:
		pr_debug("FLASH_IOC_GET_DUTY_CURRENT(%d): %d\n",
				channel, (int)fl_arg->arg);
		fl_arg->arg = aw36515_torch_current[fl_arg->arg];
		break;
	case FLASH_IOC_GET_HW_TIMEOUT:
		pr_debug("FLASH_IOC_GET_HW_TIMEOUT(%d)\n", channel);
		fl_arg->arg = aw36515_timeout_ms[channel];
		break;
	default:
		...
		struct aw36515_platform_data *pdata)
{
	...
	if (of_property_read_u32(np, "force_level", &aw36515->pdata->force_level)){
		AW36515_DEBUG("Parse no dt, force_level.\n");
	}else{
		aw36515->pdata->force_level = -1;
	}
...
aw36515_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = aw36515_parse_dt(&client->dev, chip->pdata);
	if (err)
		goto err_init_pdata;
