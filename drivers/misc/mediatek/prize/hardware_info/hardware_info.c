...
struct hardware_info current_camera_info[6] = 
{
	{"unknow","unknow","unknow","unknow"},
	{"unknow","unknow","unknow","unknow"},
	{"unknow","unknow","unknow","unknow"},
...
static void dev_get_current_camera_info(char *buf)
{
	 ...
	 //prize modify by yantaotao for camera start
	 if(strcmp(current_camera_info[5].chip,"unknow") != 0)
	 {
		p += sprintf(p, "\n[Sub2 Camera]:\n");
	 	p += sprintf(p, "  chip:%s\n", current_camera_info[5].chip);	
	 	p += sprintf(p, "  id:%s\n", current_camera_info[5].id);	
	 	p += sprintf(p, "  vendor:%s\n",current_camera_info[5].vendor);	
	 	p += sprintf(p, "  more:%s\n", current_camera_info[5].more);
	 }
	 //prize modify by yantaotao for camera end
	 len += (p - buf);  
	 HW_PRINT("%s",buf);
