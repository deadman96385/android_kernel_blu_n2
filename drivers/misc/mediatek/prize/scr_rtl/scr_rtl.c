...
static ssize_t device_scr_rtl_trans_oe_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
	struct scr_rtl_info *scr = dev_get_drvdata(dev);
	int ret = -1;
	
	if (gpio_is_valid(scr->trans_oe_gpio)){
		ret = gpio_get_value(scr->trans_oe_gpio);
	}

	return sprintf(buf, "%d\n", ret);
}
static ssize_t device_scr_rtl_trans_oe_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t size)
{
	struct scr_rtl_info *scr = dev_get_drvdata(dev);
	int trans_oe = 0;
	
	if(sscanf(buf, "%u", &trans_oe) != 1){
		dev_err(dev,"Invalid values\n");
		return -EINVAL;
	}
	dev_info(dev,"%s value = %d:\n", __func__, trans_oe);
	...
	if (gpio_is_valid(scr->trans_oe_gpio)){
		if (trans_oe){
			gpio_direction_output(scr->trans_oe_gpio, 1);
		}else{
			gpio_direction_output(scr->trans_oe_gpio, 0);
		}
	}
	return size;
}
static DEVICE_ATTR(trans_oe, 0644, device_scr_rtl_trans_oe_show, device_scr_rtl_trans_oe_store);
...
static int scr_rtl_probe(struct platform_device *pdev)
{
	...
	scr->trans_oe_gpio = of_get_named_gpio(np, "trans_oe-gpio", 0);
	if (scr->trans_oe_gpio < 0){
		dev_err(&pdev->dev, "Get rst gpio fail %d\n",scr->trans_oe_gpio);
		goto free_gpio_p03;
	}
	ret = devm_gpio_request(&pdev->dev, scr->trans_oe_gpio, "scr_rtl_trans_oe");
	if (ret){
		dev_err(&pdev->dev, "GPIO request failed\n");
		goto free_gpio_p03;
	}
	...
	if (device_create_file(scr->dev, &dev_attr_trans_oe) < 0){
		dev_err(&pdev->dev, "Create device trans_oe file fail\n");
		goto free_gpio_trans_oe;
	}
...
static int scr_rtl_remove(struct platform_device *pdev)
{
	...
	devm_gpio_free(&pdev->dev, scr->trans_oe_gpio);
	device_remove_file(&pdev->dev, &dev_attr_pwr);
	device_remove_file(&pdev->dev, &dev_attr_p03);
	device_remove_file(&pdev->dev, &dev_attr_trans_oe);
	devm_kfree(&pdev->dev, scr);
