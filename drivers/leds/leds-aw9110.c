...
{
	struct led *pled = container_of(work, struct led, brightness_work);
    struct aw9110 *aw9110 = NULL;
    unsigned char i;
	
	aw9110 = pled->aw;

    if(pled->cdev.brightness > pled->cdev.max_brightness) {
        pled->cdev.brightness = pled->cdev.max_brightness;
    }
...
{
	struct led *pled = container_of(cdev, struct led, cdev);

    pled->cdev.brightness = brightness;

    schedule_work(&pled->brightness_work);
}

static void aw9110_led_blink(struct led *pled, unsigned char blink)
{
    unsigned char i;
	struct aw9110 *aw9110 = pled->aw;
	uint8_t breath = 0;
    ...
    if(blink) {
		...
		aw9110_i2c_read(aw9110, REG_EN_BREATH, &breath);
		if (breath){
			aw9110_i2c_write(aw9110, REG_EN_BREATH, 0x00);
			breath = breath | (pled->io_set & 0x3f);
			aw9110_i2c_write(aw9110, REG_EN_BREATH, breath);
		}else{
			aw9110_i2c_write(aw9110, REG_EN_BREATH, pled->io_set & 0x3f);
		}
		//aw9110_i2c_write_bits(aw9110, REG_EN_BREATH, pled->io_set & 0x3f, pled->io_set & 0x3f);
        ...
        aw9110_i2c_write(aw9110, REG_FADE_TIME,
            (pled->fall_time<<3)|(pled->rise_time));    // fade time
        aw9110_i2c_write(aw9110, REG_FULL_TIME,
            (pled->off_time<<3)|(pled->on_time));       // on/off time
                ...
                const char *buf, size_t count)
{
    ...
    struct aw9110 *aw9110 = container_of(led_cdev, struct led, cdev)->aw;
                ...
                const char* buf, size_t len)
{
    ...
    struct led_classdev *led_cdev = dev_get_drvdata(dev);
    struct led *pled = container_of(led_cdev, struct led, cdev);

    sscanf(buf,"%d",&databuf[0]);
    aw9110_led_blink(pled, databuf[0]);
        ...
        struct device_node *np)
{
    ...
    for_each_child_of_node(np, temp) {
		pled = devm_kzalloc(aw9110->dev, sizeof(struct led), GFP_KERNEL);
		if (pled == NULL){
			continue;
		}
		pled->aw = aw9110;
		...
		ret = of_property_read_u32(temp, "aw9110,io_set",
            &pled->io_set);
        if (ret < 0) {
            dev_err(aw9110->dev,
                "Failure reading off_time, ret = %d\n", ret);
		...
		list_add(&pled->list, &aw9110->leds_list);
		
		INIT_WORK(&pled->brightness_work, aw9110_brightness_work);

		pled->cdev.brightness_set = aw9110_set_brightness;
		ret = led_classdev_register(aw9110->dev, &pled->cdev);
		if (ret) {
			dev_err(aw9110->dev,
		...
		ret = sysfs_create_group(&pled->cdev.dev->kobj,
...
static int aw9110_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	INIT_LIST_HEAD(&aw9110->leds_list);
